...
class CoT(dspy.Module):
    def __init__(self):
        super().__init__()
        self.candidate_programs = dspy.ChainOfThought("question -> answer")
    
    def forward(self, question):
        return self.candidate_programs(question=question)
...
optimized_cot = teleprompter.compile(CoT(), trainset=gsm8k_trainset, valset=gsm8k_devset, max_demos=1)
ensemble_optimizer = Ensemble(reduce_fn=dspy.majority)
programs = [x[-1] for x in optimized_cot.candidate_programs]
your_dspy_program_compiled_ensemble = ensemble_optimizer.compile(programs[:3])
# from dspy.predict import KNN
breakpoint()
your_dspy_program_compiled_ensemble("Amanda has to sell 80 tickets in 3 days so she can make enough money to go on vacation. On the first day, she sells 5 of her friends 4 tickets each. On the second day she sells 32 tickets. How many tickets does she need to sell on the third day to meet her goal?")
